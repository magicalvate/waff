...
class Conv(Layer):
    ...
    def __init__(self,kernelnumber,kernelsizes,padding='valid',strides=1,lr_base=[1,2],name = '',kernel_initializer ='GlorotNormal' ,bias_initializer ='Zeros',activation = None):
        Layer.__init__(self,lr_base,'CONV',name,kernel_initializer,bias_initializer,activation)
        self.outputChannelSize = kernelnumber
    ...
    def passForward(self):
        ...
        if self.variableNeedInitialize:
            self.initiate_variable(self.outputChannelSize,self.inputChannelSize,self.kernelheight,self.kernelwidth)
        ...
        self.a_= Conv2d(padded_map,self.w_,self.b_,self.strides)
    ...
    def __calculateDeltaBackLayer(self,delta):
        ...
        deltaBackLayer = Conv2d(delta_padded,w_rot180,mode='delta_back')
        if self.padding!=0:
    ...
    def backpropagate(self,delta,backLayerDelta=True):
        ...
        delta_w_ = Conv2d(padded_inputFeatureMaps,delta,mode = 'delta_w')
